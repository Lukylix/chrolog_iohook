...
namespace chrolog_iohook
{
  ...
  struct arguments
  {
    ...
    bool timestamp_every; // log timestamp for every key, --timestamp-every switch
    ...
    std::string mouse_device;     // user-specified input event device, given with -d switch
    ...
    while ((c = getopt_long(argc, argv, "sm:o:ukd:?", long_options, &option_index)) != -1)
    {
      ...
      case 0:
        ...
        case FLAG_POST_SIZE:
          ...
          case 'M':
          case 'm':
            args.post_size *= 1000000;
            break;
          }

        case FLAG_TIMESTAMP_EVERY:
          args.timestamp_every = true;
          break;
        }
        break;
